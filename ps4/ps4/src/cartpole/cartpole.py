...
def choose_action(state, mdp_data):
    ...
    values = mdp_data['value']
    opt_0, opt_1 = np.sum(values*trans_prob[state,:,0]), np.sum(values*trans_prob[state,:,1])
    if opt_0 == opt_1:
        return 0 if np.random.uniform() < 0.5 else 1
    else:
        return np.argmax([opt_0, opt_1])
    # *** END CODE HERE ***
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    # *** START CODE HERE ***
    count_total = [np.sum(mdp_data['transition_counts'][:,:,0], axis=1), np.sum(mdp_data['transition_counts'][:,:,1], axis=1)]
    tried = [count_total[0] != 0, count_total[1] != 0]
    #if tried before:
    mdp_data['transition_probs'][:,:,0][tried[0]] = (mdp_data['transition_counts'][:,:,0][tried[0]]/
                                                            count_total[0][tried[0]].reshape((-1,1)))
    mdp_data['transition_probs'][:,:,1][tried[1]] = (mdp_data['transition_counts'][:,:,1][tried[1]]/
                                                            count_total[1][tried[1]].reshape((-1,1)))
    
    reached = mdp_data['reward_counts'][:,1] != 0
    mdp_data['reward'][reached] = -1. * mdp_data['reward_counts'][:,0][reached]/mdp_data['reward_counts'][:,1][reached]
    # *** END CODE HERE ***
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    iteration = 0
    values = mdp_data['value']
    rewards = mdp_data['reward']
    prob = mdp_data['transition_probs']
    new_values = values.copy()
    while True:
        #asynchronous vs synchronous
        # for state, value in enumerate(new_values):
        #     new_values[state] = rewards[state] + gamma * np.max([np.sum(new_values*prob[state,:,0]),
        #                                                               np.sum(new_values*prob[state,:,1])])
        new_values = rewards + gamma * np.max(np.array([np.sum(values*prob[:,:,0], axis = 1), 
                                                        np.sum(values*prob[:,:,1], axis = 1)]), axis = 0)
        if np.max(np.abs(new_values - values)) < tolerance:
            mdp_data['value'] = values
...
def main(plot=True):
    ...
    if plot:
        ...
        plt.savefig('./control_'+ str(seed) +'.pdf')
