...
def initial_state():
    ...
    # *** START CODE HERE ***
    #The state design is a dictionary with keys as update step and the value a list size 2 with
    #the first element as the beta_i and the second element containing the coordinates of the corresponding
    #input x_i
...
def predict(state, kernel, x_i):
    ...
    """
    # *** START CODE HERE ***
    g_i = np.sum([state[i][0]*kernel(state[i][1], x_i) for i in range(len(state))])
    return sign(g_i)
    # *** END CODE HERE ***
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    """
    # *** START CODE HERE ***
    k = kernel(state[0][1], x_i)
    g_i = np.sum([state[i][0]*kernel(state[i][1], x_i) for i in range(len(state))])
    state[len(state)] = [learning_rate*(y_i - sign(g_i)), x_i]
    # *** END CODE HERE ***
